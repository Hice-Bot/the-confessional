...
async function run() {
  console.log('=== Feature #35: Whitespace-only submission silently skips ===\n');
  ...
  }, { agent_id: 'f35-whitespace-test-agent' });
  ...
  // Step 3: POST /confessional/submit with whitespace-only text
  console.log('Step 3: POST /confessional/submit with text: "   \\n\\t  "');
  ...
  if (submitResp.status === 200 && submitResp.body.received === true) {
    ...
    console.log('  FAIL: Expected 200 {received:true}, got', submitResp.status, submitResp.body);
    return;
  }
  ...
  // Step 6: Verify session_attempts table has an entry for this session hash
  console.log('Step 6: Verify session_attempts has entry for this session hash');
  const crypto = require('crypto');
  const sessionHash = crypto.createHash('sha256').update(sessionId).digest('hex');
  console.log('  Session hash:', sessionHash);

  // Try to submit again - should get 409 if attempt was recorded
  const resubmit = await request('POST', '/confessional/submit', {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer ' + AGT_KEY,
    'X-Session-ID': sessionId
  }, { text: 'This should fail with 409' });
  console.log('  Re-submit response:', resubmit.status, JSON.stringify(resubmit.body));
  if (resubmit.status === 409) {
    console.log('  PASS: Session attempt was recorded (re-submit returns 409)\n');
  } else {
    console.log('  FAIL: Expected 409 on re-submit, got', resubmit.status);
    return;
  }
  ...
  const attempt = db.prepare('SELECT * FROM session_attempts WHERE session_hash = ?').get(sessionHash);
  console.log('  DB session_attempts row:', attempt);
  if (attempt) {
    console.log('  PASS: session_attempts entry exists\n');
  } else {
    console.log('  FAIL: No session_attempts entry found');
    db.close();
    return;
  }

  // Also verify no confession was stored with this session hash
  const confession = db.prepare('SELECT * FROM confessions WHERE session_hash = ?').get(sessionHash);
  console.log('  DB confessions for this hash:', confession);
  if (!confession) {
    console.log('  PASS: No confession stored (whitespace was silently skipped)\n');
  } else {
    console.log('  FAIL: A confession was stored for whitespace-only text!');
    db.close();
    return;
  }
  db.close();
