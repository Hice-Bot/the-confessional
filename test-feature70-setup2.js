...
const needed = Math.max(0, 26 - currentCount.cnt);
...
const insertAll = db.transaction((count) => {
  for (let i = 0; i < count; i++) {
    ...
    const text = 'LIMIT_TEST_70_NUM_' + String(i + 1).padStart(3, '0') + ' This is a test confession for verifying the default limit of 20.';

    // Stagger timestamps so ordering is clear
    const offset = count - i;
    const timestamp = new Date(Date.now() - offset * 1000).toISOString().replace('T', ' ').replace('Z', '');
